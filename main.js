...
const invalid1 = [7, 5, 3, 2, 7, 7, 8, 7, 7, 1, 0, 9, 1, 7, 9, 5];
  ...
  for (let i = cardNumber.length - 1; i >= 0; i--) {
    ...
    if (i > 0) {
      if (cardNumber[i] * 2 >= 10) {
        sumChecker = sumChecker + (cardNumber[i] - 9);
      } else sumChecker = sumChecker + cardNumber[i];
    }
  }
  if (sumChecker % 10 === 0) {
    console.log('Card is Valid. card number: ' + cardNumber + '         //Valid');
    return true;
  } else {
    console.log('Card is not Valid, check and try again.' + 'card number: ' + cardNumber+ '         //Invalid');
    return false;
  }
};
